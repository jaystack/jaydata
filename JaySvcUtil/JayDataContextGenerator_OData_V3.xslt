<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    ...
    <xsl:for-each select="//edm:EntityType | //edm:ComplexType" xml:space="default">
      <xsl:message terminate="no">
        Info: generating type <xsl:value-of select="concat(../@Namespace, '.', @Name)"/>
      </xsl:message>
  ...
  <xsl:template match="edm:FunctionImport">
    ...
    </xsl:for-each>], method: '<xsl:value-of select="@m:HttpMethod"/>' })
  </xsl:template>
  ...
  <xsl:template match="edm:EntitySet">
    '<xsl:value-of select="@Name"/>': { type: <xsl:value-of select="$EntitySetBaseClass"  />, elementType: <xsl:value-of select="@EntityType"/> }
  </xsl:template>

  <xsl:template match="edm:Property | edm:NavigationProperty">
    <property>
      <xsl:variable name="memberDefinition">
        <xsl:if test="parent::edm:EntityType/edm:Key/edm:PropertyRef[@Name = current()/@Name]">
          <key>true</key>
        </xsl:if>
        <xsl:apply-templates select="@*[local-name() != 'Name']" mode="render-field" />
      </xsl:variable>'<xsl:value-of select="@Name"/>': { <xsl:choose>
        <xsl:when test="function-available('msxsl:node-set')">
          <xsl:for-each select="msxsl:node-set($memberDefinition)/*">
        ...
        <xsl:otherwise>
          <xsl:for-each select="exsl:node-set($memberDefinition)/*">
            <xsl:value-of select="name()"/>:<xsl:value-of select="."/>
            <xsl:if test="position() != last()">
              ,<xsl:text> </xsl:text>
            </xsl:if>
  ...
  <xsl:template match="@ConcurrencyMode" mode="render-field">
    <concurrencyMode>
      $data.ConcurrencyMode.<xsl:value-of select="."/>
    </concurrencyMode>
  </xsl:template>
  ...
  <xsl:template match="@Relationship" mode="render-field">
    ...
    <xsl:choose>
      <xsl:when test="$m = '*'">
        <type>
          '<xsl:value-of select="$CollectionBaseClass"/>'
        </type>
        <elementType>
        ...
        <xsl:if test="$otherProp/@Name">
          <inverseProperty>
            '<xsl:value-of select="$otherProp/@Name"/>'
          </inverseProperty>
        </xsl:if>
      </xsl:when>
      <xsl:when test="$m = '0..1'">
  ...
  <xsl:template match="*" mode="render-field">
    ...
    <xsl:message terminate="no">
      Warning: <xsl:value-of select="../../@Name"/>.<xsl:value-of select="../@Name"/>:<xsl:value-of select="name()"/> is an unknown/unprocessed attribued
    </xsl:message>
  </xsl:template>
  <!--<xsl:template match="*">
    !<xsl:value-of select="name()"/>!
