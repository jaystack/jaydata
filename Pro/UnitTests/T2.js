$(document).ready(function () {
    ...
    SimpleTransactionTests({ name: "oData", databaseName: 'T1', oDataServiceHost: "/Services/emptyNewsReader.svc", serviceUrl: '/Services/oDataDbDelete.asmx', dbCreation: $data.storageProviders.DbCreationType.DropAllExistingTables }, '_oData');
    TransactionTests({ name: "oData", databaseName: 'T1', oDataServiceHost: "/Services/emptyNewsReader.svc", serviceUrl: '/Services/oDataDbDelete.asmx', dbCreation: $data.storageProviders.DbCreationType.DropAllExistingTables }, '_oData');
    ...
    if ($data.StorageProviderLoader.isSupported('sqLite')) {
        SimpleTransactionTests({ name: "sqLite", databaseName: 'transactionTests', dbCreation: $data.storageProviders.DbCreationType.DropAllExistingTables }, '_Web_SQL');
    ...
    if ($data.StorageProviderLoader.isSupported('indexedDb')) {
        SimpleTransactionTests({ name: "indexedDb", databaseName: 'transactionTests', dbCreation: $data.storageProviders.DbCreationType.DropAllExistingTables }, '_indexedDb');
