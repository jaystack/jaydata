...
$data.IndexedDBConverter = {
    ...
    toDb: {
        ...
        '$data.Int32': $data.Container.proxyConverter,
        '$data.Number': $data.Container.proxyConverter,
        // auto convertion for JSON-WebService Date format {"now":"2013-10-21T13:28:06.419Z"}
        '$data.Date': function(d) { return typeof d === 'string' ? new Date(d) : d; },
        '$data.DateTimeOffset': $data.Container.proxyConverter,
        '$data.Time': $data.Container.proxyConverter,
        '$data.String': $data.Container.proxyConverter,
        '$data.Boolean': $data.Container.proxyConverter,
...
$data.Class.define('$data.storageProviders.indexedDb.IndexedDBStorageProvider', $data.StorageProviderBase, null,
{
    ...
    bulkInsert: function (entitySet, fields, datas, callback) {
        ...
        try {
            var self = this;
            var publicMappedProperties = entitySet.elementType.memberDefinitions
                .getPublicMappedProperties();
            var convertedItems = datas.map(function (item) {
                item.physicalData = {};
                publicMappedProperties.forEach(function (memDef) {
                    var typeName = Container.resolveName(memDef.type);
                    if (memDef.key && memDef.computed && item[memDef.name] == undefined) {
                        if (typeof self.supportedAutoincrementKeys[typeName] === 'function') {
                            var keyValue = self.supportedAutoincrementKeys[typeName]();
                            item[memDef.name] = self.fieldConverter.toDb[typeName](keyValue);
                        } else {
                            // Autogenerated fields for new items should not be present in the physicalData
                            return;
                        }
                    }
                    if (!memDef.inverseProperty && typeof memDef.concurrencyMode === 'undefined') {
                        if (self.fieldConverter.toDb[typeName]) {
                            item.physicalData[memDef.name] = self.fieldConverter.toDb[typeName](item[memDef.name]);
                        } else {
            ...
            var stores = [entitySet.tableName];

            tran = self.db.transaction(stores, self.IDBTransactionType.READ_WRITE).setCallbacks({
            ...
            var store = tran.objectStore(entitySet.tableName);
