...
function TypeTests(providerConfig, msg) {
    ...
    test('filter sbyte', function () {
        var itemNumber = 4;
        expect(1 + itemNumber);

        _getDataContext(4)
            .then(function (context) {
                context.TestItemTypes.filter('it.sb0 == 10').toArray(function (item) {
                    var count = item.length;
                    equal(item[0].sb0, 10, 'value');

                    context.TestItemTypes.filter('it.sb0 != 10').toArray(function (oItem) {
                        for (var i = 0; i < oItem.length; i++) {
                            notEqual(oItem[i].sb0, 10, 'opposite value');
                        }
                        equal(count + oItem.length, itemNumber, 'data length');
    ...
    test('filter datetimeoffset', function () {
        var itemNumber = 4;
        expect(1 + itemNumber);

        _getDataContext(4)
            .then(function (context) {
                context.TestItemTypes.filter('it.dto < "2100/05/04"').toArray(function (item) {
                    var count = item.length;
                    equal(item[0].dto.valueOf(), new Date(2100, 0, 1).valueOf(), 'value');

                    context.TestItemTypes.filter('it.dto > "2100/05/04"').toArray(function (oItem) {
                        for (var i = 0; i < oItem.length; i++) {
                            notEqual(oItem[i].dto.valueOf(), new Date(2100, 0, 1).valueOf(), 'opposite value');
                        }
                        equal(count + oItem.length, itemNumber, 'data length');
