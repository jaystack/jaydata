...
function convertTestFail(value) {
  return function (type, testText) {
    it(testText, () => {
      // expect(model.get.bind(model, 'z')).to.throw("'"TypeError: Value '$data.String' not convertable to '$data.Integer'");
      try {
        $data.Container.convertTo(value, type);
        expect(true).to.equal(false, 'Test not failed from type ' + (type.fullName || type.name));
      } catch (e) {
...
describe('Convert tests', () => {
  describe('$data.Boolean', () => {
    convertTo($data.Boolean, {
  ...
  describe('$data.String', () => {
    convertTo($data.String, {
      ...
      'from integer number': convertTest(42, '42'),
      'from float number': convertTest(3.14, '3.14'),
      'from bigint': convertTest(0xffffffff, ''+(0xffffffff)),
      'from Int32 max value': convertTest(0x7fffffff, ''+(0x7fffffff)),
      'from Int32 max value + 1': convertTest(0x80000000, ''+(0x80000000)),
      'from valid string ': convertTest('123', '123'),
      'from valid decimal string': convertTest('123123123123.123123123123', '123123123123.123123123123'),
      'from valid bigint string': convertTest('123123123123', '123123123123'),
      'from valid float string': convertTest('3.14', '3.14'),
